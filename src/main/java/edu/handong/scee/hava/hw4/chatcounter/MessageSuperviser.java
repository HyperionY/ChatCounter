...
public class MessageSuperviser {
	String aline = "";
	int line = 0;
	...
	public void parseMessage() {
		...
		for(int i = 0; i < lines.size(); i++) {
			...
			if(aline.matches(pattern1)) {
				Pattern p = Pattern.compile(subpattern1);
				Matcher matcher = p.matcher(aline);
				if(matcher.find()) {
			...
			} else if(aline.matches(pattern2)) {
			...
			if(aline.matches(pattern3)) {
				...
				if(matcher.find()) {
					...
					checker.mapUser(nickname, datetime, message);
				}
